...
from scraper.police_tools.police_core import ScrapeLogData, NextUrl, ListingScrapeMetadata
...
class ListingsParser(BaseScraper):
    ...
    GOV_SITE = True
    SEMAPHORE_COUNT = 12 # One for each municipality
    DEFAULT_CRON_MAX_PAGES = 5
    ...
    def get_next_page(
        ...
        """ Return the 'next_page' url or None to indicate the end of listing.
        """
        ...
        if next_page:
            ...
            self.logger.debug(f"Continuing to page "
                              f"{metadata.pages_scraped}/{metadata.listing_max_pages if not cron_max_pages else cron_max_pages}"
    ...
    async def parse_listing(
        ...
        """ Parse the html content of the current articles listing page. \n
            ...
            Returns the url for the next page to parse, if "None" (i.e. no more pages) then the scraper stops.
        """
        ...
        # Return the next page link, which changes 'new_url' in 'scrape_municipality' to continue the loop
    ...
    def mk_full_coros(self, archives):
        """ Returns prepped coroutines to scrape all archive pages from the INPUT_FILE.
        """
    ...
    def mk_cron_coros(self, archives, cron_max_pages: int):
        """ Returns coroutines for the cron scrape. Only current year.
        """
        current_year = str(datetime.now().year) # todo this might silently fail on New Years?
        article_scrape_coros = [
    ...
    async def scrape(self, cron_max_pages: int | None) -> bool:
        """ Main orchestrator, runs archive jobs (to get year links of archives), then the article jobs (to get article urls).
            Returns a bool to indicate if new articles were scraped.
        """
        if cron_max_pages:
            self.logger.info(f"Running '{self.MODULE_NAME}' in CRON mode, pages to check: {cron_max_pages}.")

        pre_scrape_arts_count = len(await self.set_existing_urls())
        ...
        # Return true if found any new articles
        post_scrape_parse_arts = await self.set_existing_urls()
        found_new_articles = pre_scrape_arts_count != len(post_scrape_parse_arts)
