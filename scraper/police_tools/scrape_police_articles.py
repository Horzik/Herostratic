...
class PoliceArticlesScraper(BaseScraper):
    ...
    def load_czech_locations(self):
        ...
        # The linter warnings are wrong here, cuz this works
    ...
    def log_report(self, elapsed_time):
        ...
        self.logger.info(f"Got {self.stats.articles_processed} articles, successfully saved {self.stats.saved_articles}.")
        ...
        self.logger.info(f"In total there are currently {len(self.results_cache)}")
        self.logger.info(f"Exiting...")

    @staticmethod
    def write_failed_article(res_url):
        """ Note: we could store this as set in cache then write on exit, but we shouldn't write fails too often anyway
        """
    ...
    def resolve_archive_category(self, arch_cat, date_text, soup, url, region):
        """ Fed 'arch_cat' can be either a year or the string 'non_years'. Try resolving to a year.
        """
    ...
    def select_element_lists(self, soup: BeautifulSoup, url: str, region: str, arch_cat: str):
        ...
        for name, element in required.items():
            if not element:
                ...
                raise ValueError(f"ValueError: Missing '{name}' from '{url}'")
    ...
    @staticmethod
    def find_youtube_links(text: str) -> list[str] | None:
        """ Goes through the whole soup and looks for 'youtube' links, returns them in a list.
        """
        # Pattern to get the url path for any direct link or inside the iframe (i.e. 'youtube-nocookie')
    ...
    def parse_docs_links(self, docs_el, sound_el=None) -> set[FileMetadata]:
        ...
        if sound_el is not None:
            ...
            if sound_file_path:
                ...
                # "soubor/vandal-2109-mp3.aspx" ==> we get just name in middle => "vandal-2109-mp3"
                sound_file_name = sound_file_path.rsplit('.')[0].rsplit('/')[1]
    ...
    async def download_files(self, imgs_el, docs_el, sound_el, soup, url) -> list[tuple[str, str]]:
        ...
        # Collect the article file links
        gallery_links = set()
        docs_links = set()
        if imgs_el:
        ...
        ytb_urls = self.find_youtube_links(str(soup))
        if ytb_urls:
            if not file_dir_abs_path.is_dir():
                file_dir_abs_path.mkdir(parents=True, exist_ok=True)
    ...
    async def flush_buffer(self):
        """ Add results to cache . Then it saves the batched articles.
        """
        async with self.lock:
            for region, arch_cat, content in self.results_buffer:
                self.results_cache.setdefault(region, {}).setdefault(arch_cat, []).append(content)

            # Write results back and clean the buffer
            atomic_json_write(self.results_cache, self.OUTPUT_FILE)
            self.results_buffer = []
        return

    async def fetch_page(self, url: str) -> tuple[BeautifulSoup, bytes]:
        ...
        if police_404:
            # todo only 404s have 'h1', but probably the text check doesn't pass
        ...
        return soup, page_bytes

    async def scrape_article(self, url: str, region: str, arch_cat: str) -> PoliceArticleResult:
        """ The main scraping method. Receives the
        """
        soup, page_bytes = await self.fetch_page(url)
        ...
        # For 'content_text' we first define unwanted elements, then exclude those from the text parse
        ...
        # Get keywords based on url and content
        keywords = {
    ...
    def load_result_urls(self) -> set[str]:
        """ Reads the "police_results" and returns a set of the URLs.
            Used for deduping results, meaning to avoid re-scraping a result we already have.
        """
        ...
        self.results_cache = results_dict
    ...
    async def run(self):
        """ Main run method. Awaits the queue tasks.
        """
